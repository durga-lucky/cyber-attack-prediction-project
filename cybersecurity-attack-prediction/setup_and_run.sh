...
# Cybersecurity Attack Prediction - Setup & Run Script (Mac/Linux)

echo "==========================================================="
echo "  Cybersecurity Attack Prediction - Setup & Run Script"
echo "==========================================================="
echo ""
...
cat > ".vscode/tasks.json" <<EOL
{
    ...
    "tasks": [
        ...
        },
        {
            ...
            "type": "shell",
            "command": "cd backend && pip3 install -r requirements.txt && cd .. && npm install",
            "options": {
                "cwd": "\${workspaceFolder}"
            },
            "problemMatcher": [],
...
if [ -f "backend/requirements.txt" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "[WARNING] Backend dependencies might have failed to install."
        echo "Trying with 'pip' instead of 'pip3'..."
...
if command -v mongosh &> /dev/null; then
    mongosh --eval "db.version()" --quiet mongodb://127.0.0.1:27017/cybersecurity_app &> /dev/null
    if [ $? -ne 0 ]; then
    ...
    mongo --eval "db.version()" --quiet mongodb://127.0.0.1:27017/cybersecurity_app &> /dev/null
    if [ $? -ne 0 ]; then
        echo "[WARNING] Could not connect to MongoDB at mongodb://127.0.0.1:27017"
        echo "   Make sure MongoDB is running before starting the backend."
    else
...
echo "    1. Backend Server (localhost:8000)"
